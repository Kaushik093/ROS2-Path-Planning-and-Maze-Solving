import cv2
import numpy as np
from maze_bot.utilities import ret_smallest_obj, ret_largest_obj
...
class bot_localizer():
    ...
    def ret_rois_boundinghull(rois_mask, cnts):
        ...
        cnts_largest = cv2.findContours(
            maze_enclosure, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)[0]  # OpenCV 4.2
    ...
    def extract_bg(self, frame):
        ...
        if min_cntr_idx != -1:
            ...
            self.bg_model = cv2.bitwise_and(
                Ground_replica, Ground_replica, mask=car_mask)
        ...
        self.maze_og = cv2.rotate(
            maze_occupencygrid, cv2.ROTATE_90_COUNTERCLOCKWISE)
    ...
    def localize_bot(self, curr_frame, frame_disp):
        ...
        car_circular_mask = cv2.circle(car_mask.copy(), (int(
            center[0]), int(center[1])), int(radii+(radii*0.4)), 255, 3)
