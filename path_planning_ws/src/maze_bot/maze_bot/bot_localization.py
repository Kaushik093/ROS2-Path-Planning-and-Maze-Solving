import cv2
import numpy as np
from .utilities import imfill, ret_largest_obj, ret_smallest_obj
...
class bot_localizer():
    ...
    def ret_rois_boundinghull(rois_mask,cnts):
        ...
        img,cnts_largest,hier= cv2.findContours(maze_enclosure,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)
    ...
    def extract_bg(self,frame):
        ...
        i,cnts,h = cv2.findContours(edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    ...
    def localise_bot(self,curr_frame,frame_disp):
        ...
        cv2.imshow("car localized",frame_disp)
    
            
''' 
1. Cvt to gray and extract edges
2. Find contours
3. Loop over contours and draw on empty placeholder'''