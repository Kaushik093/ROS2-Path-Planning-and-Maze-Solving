...
# Interest point : A point where a decison has to be made to move forward or turn
...
class Graph():
    ...
    def add_vertex(self,vertex,neighbor=None,case=None,cost=None):
        # print("Adding vertex {} with neighbor {} and case {}".format(vertex,neighbor,case))
...
class bot_mapper():
    ...
    def __init__(self):
        ...
        self.crp_amt =5 # remove 5 pixels from all sides

        self.Graph=Graph()

        #State variables for connection status of  each vertex :
    ...
    def get_surround_pixel_intensities(maze,curr_row,curr_col):
        ...
        no_of_pathways =  (top_left + top  + top_rgt  + lft  + 0  + rgt      +  btm_left + btm  + btm_rgt   )
    ...
    def one_pass(self,maze):  # Extract interest points from the maze
        ...
        # cv2.namedWindow("Maze (IP)",cv2.WINDOW_FREERATIO)
        rows =maze.shape[0]
        cols =maze.shape[1]
        ...
        for row in range(rows):
            for col in range(cols):
                if maze[row,col] == 255:
                    ...
                    #Probable IP =  Find surrounding pixels intensities
                    top_left,top,top_rgt,rgt,btm_rgt,btm,btm_left,lft,paths = self.get_surround_pixel_intensities(maze.copy(),row,col)
                    # print("Surrounding intensity : ",top_left,top,top_rgt,rgt,btm_rgt,btm,btm_left,lft,paths)

                    # Check if we are at start or end point
                    ...
                    # Check if turn or ordinary path :
                    elif (paths==2):
                        # print("paths2")
                        crop = maze[row-1:row+2,col-1:col+2]
                        ...
                        if draw_intrstpts:
                            maze_bgr= cv2.circle(maze_bgr, (col,row), 1, (255,0,0),2)
                        cv2.imshow("Maze(IP)",maze_bgr)
    ...
    def graphify(self,extracted_maze):        #Extracted maze is received from the localization stage
         ...
         if not self.graphified:
            ...
            thinned_cropped=thinned[self.crp_amt:thinned.shape[0]-self.crp_amt,self.crp_amt:thinned.shape[1]-self.crp_amt]
            cv2.imshow("Cropped Thinned Maze",thinned_cropped)

            extracted_maze_cropped=extracted_maze[self.crp_amt:extracted_maze.shape[0]-self.crp_amt,self.crp_amt:extracted_maze.shape[1]-self.crp_amt]
